...
from TOKENSFILE import TelegramToken # Уберите, я использовал для подтягивания токена из другого файла
...
class ToolsAndFunction:
    ...
    def generate_bot(self, selected_dishes=None):
    ...
    def detect_product(self, image: str):
    ...
    def forming_string_from_products(self):
    ...
    def add_products_in_string(self, message):
...
TOKEN = TelegramToken()  # '7572423155:AAG_9aYLKolLTdXlND9ibGLiDHJNEAq8XDw'  вот это написать вместо TelegramToken()
dp = Dispatcher()
bot = Bot(token=TOKEN)
Bot = ToolsAndFunction(dp, bot)
